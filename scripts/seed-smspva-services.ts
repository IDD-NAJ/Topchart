...
async function main() {
  ...
  try {
    ...
    `);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_smspva_services_active   ON smspva_services(is_active)`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_smspva_services_category ON smspva_services(category)`);
    ...
    for (const svc of SMSPVA_SERVICES) {
      const result = await client.query(
        ...
        [svc.code, svc.name, svc.category, svc.baseUsdPrice]
      );
      if (result.rows[0]?.was_inserted) inserted++;
    }

    const total = await client.query(`SELECT COUNT(*) AS cnt FROM smspva_services`);
    console.log(`  ✓ ${inserted} new rows inserted`);
    console.log(`  ✓ ${total.rows[0].cnt} total services in DB`);
    console.log("\nDone ✅");
  } finally {
    client.release();
    await pool.end();

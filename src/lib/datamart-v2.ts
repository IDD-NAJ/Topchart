...
export async function submitDatamartPurchase(params: {
  ...
  if (!result.ok || !result.data) {
    ...
    if (DUPLICATE_ORDER_PATTERN.test(errorMessage)) {
      logger.warn({ message: "[DataMart] Duplicate order detected — treating as pending", idempotencyKey: params.idempotencyKey, providerMessage: errorMessage });
      await sql`
      ...
      return {
        ...
        balanceAfter: 0,
        orderStatus: "pending",
        processingMethod: "duplicate_in_progress",
      };
    }

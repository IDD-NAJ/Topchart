...
async function GETHandler(request: NextRequest) {
  try {
    ...
    const activeDataBundles = await sql`
      SELECT id, network_id, name, price, price_override, is_active 
      FROM data_bundles
    ...
    const formattedDataBundles = activeDataBundles.map((b: any) => ({
      id: b.id,
      network: b.network_id.toLowerCase(),
      name: b.name,
      size: b.name,
    ...
    const uniqueNetworks = [...new Set(activeDataBundles.map((b: any) => b.network_id.toLowerCase()))];

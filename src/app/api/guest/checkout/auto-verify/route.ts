...
export async function POST(request: NextRequest) {
  try {
    ...
    if (paystackStatus !== "success") {
      await updateGuestOrderPayment(
        order.id,
        paystackStatus as "failed" | "abandoned",
        {
      ...
      if (
        ...
        try {
          ...
          sendGuestOrderConfirmationEmail({
            ...
            console.error("[GuestCheckout/auto-verify] Email error:", err)
          );
          ...
          console.error("[GuestCheckout/auto-verify] DataMart fulfillment error:", err);
          await updateGuestOrderFulfillment(order.id, "failed", {
          ...
          sendGuestOrderConfirmationEmail({
            ...
            console.error("[GuestCheckout/auto-verify] Email error:", emailErr)
          );

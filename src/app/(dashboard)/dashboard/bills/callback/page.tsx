...
function CallbackInner() {
  ...
  useEffect(() => {
    ...
    const verify = async (): Promise<void> => {
      if (cancelled) return
      try {
        const res = await fetch(`/api/purchases/bills/verify?reference=${encodeURIComponent(reference)}`)
        ...
        if (data?.success && status === "success") {
          setPhase("success")
          router.replace(`/dashboard?bill_success=1&reference=${encodeURIComponent(reference)}`)
          return
        }
        ...
        <CardContent className="p-8">
          {phase === "verifying" || phase === "pending" ? (
            ...
            <div className="space-y-4 text-center">
              ...
              <h2 className="text-xl font-semibold">Could not complete payment</h2>
...
export default function BillsCallbackPage() {
  return (
    <Suspense
      fallback={

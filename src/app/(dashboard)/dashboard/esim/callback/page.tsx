...
function CallbackInner() {
  ...
  useEffect(() => {
    ...
    const verify = async (): Promise<void> => {
      if (cancelled) return
      try {
        const res = await fetch(`/api/purchases/esim/verify?reference=${encodeURIComponent(reference)}`)
        ...
        <CardContent className="p-8">
          {phase === "verifying" || phase === "pending" ? (
            ...
            <div className="flex flex-col items-center gap-4 text-center">
              ...
              <p className="text-sm text-muted-foreground">Taking you back to your eSIM orders.</p>
            </div>
          ) : (
            <div className="space-y-4 text-center">
              <XCircle className="h-12 w-12 text-red-500 mx-auto" />
              <h2 className="text-xl font-semibold">Could not complete order</h2>
...
export default function EsimCallbackPage() {
  return (
    <Suspense
      fallback={

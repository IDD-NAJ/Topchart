...
async function createAdminAccount() {
  ...
  try {
    ...
    const existingAdmin = await sql`
      SELECT id, email FROM users WHERE email = 'najeebiddrisu79@gmail.com'
    `;
    
    if (existingAdmin.length > 0) {
      console.log('ℹ️  Admin account already exists with ID:', existingAdmin[0].id);
    ...
    // Create admin user without updatedAt column
